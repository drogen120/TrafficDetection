...
tf.app.flags.DEFINE_integer('max_number_of_steps', 900000,
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        summaries.add(tf.summary.image("input_image",
                                       tf.cast(b_image, tf.float32)))
        f_i = 0
        for gt_map in b_gscores:
            gt_features = tf.reduce_max(gt_map, axis=3)
            gt_features = tf.expand_dims(gt_features, -1)
            summaries.add(tf.summary.image("gt_map_%d" %
                                           f_i, tf.cast(gt_features, tf.float32)))
            f_i += 1
        arg_scope = ssd_net.arg_scope(
            weight_decay=FLAGS.weight_decay, data_format=DATA_FORMAT)
        ...
        for predict_map in predictions:
            ...
            summaries.add(tf.summary.image("predicte_map_%d" %
                                           f_i, tf.cast(predict_map, tf.float32)))
            f_i += 1
